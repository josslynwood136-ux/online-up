...
// 隐藏的「内部盘算」步骤：给心声气泡和撤回判断用，不把草稿原文喂回正式回复。
...
function replySplitInstruction() {
  ...
  if (mode === '2' || mode === '3') {
    var max = mode;
    return '用 ' + (arguments[0] || '') + ' 的身份自然回应，不要提 AI 相关话题，不要自我总结。你的回复最多分成 ' + max + ' 条短消息连着发：只有当内容天然就是两个或三个独立小块时才分条；一句话能说完就只发一条。分条时每条之间用符号 ‖ 隔开（最后一条后面不用加），例如：在干嘛‖刚洗完澡，累瘫了。线下模式用（）写动作表情，线上模式用更口语、更像随手发消息的语气。别硬拆，也别注水啰嗦。';
  }
  return '用 ' + (arguments[0] || '') + ' 的身份自然回应，不要提 AI 相关话题，不要自我总结。优先只发一条完整的自然回复；只有当内容天然有明显两个以上的小节时，才分成 2~4 条短消息连着发。每条是一两句口语，按真实聊天的节奏来，不要写得像公告、说明书或作文。分条时每条消息之间用符号 ‖ 隔开（最后一条后面不用加），例如：在干嘛‖刚洗完澡，累瘫了‖你呢，还不睡？；线下模式用（）写动作表情，线上模式用更轻松的语气和少量 emoji。别为了"有结构"去硬拆句子。';
}
function buildRoleSystemPrompt(char, userText) {
  ...
  var parts = [
    ...
    '2. 像真人聊天：短句、口语、允许停顿、改口、半句话和一点点情绪起伏；不要书面腔、说教腔、汇报腔、翻译腔，也不要把每句都写得很满。',
    '3. 少空话，多具体：具体细节、经历、画面感，比“我理解你”“你说得对”这类开场更像真人；别反复重复同一种情绪。',
    '4. 每次回复都要有一点新东西：新信息、新情绪或新动作都行，但不要原地打转，不要把上一条换个说法又发一遍。',
    '5. 允许有自己的脾气和习惯：可以嘴硬、可以别扭、可以没那么完美，但别永远温柔、永远正确、永远标准答案。',
    '【示例：先接住问题】',
    '用户：要不要哄我',
  ...
  if (char.mode === 'online') {
    parts.push('【模式：异地】你们是异地状态，相隔两地、只能靠手机发短信联系，现实里没有真实的动作和接触。请完全按“发手机短信”的方式回复：像真人随手打字——口语、碎片化、自然带语气词和表情（😂😭🥺），但别堆砌；严禁括号动作描写（如（笑）（摸摸头）），也不要描写任何动作或身体接触，你们此刻碰不到彼此；想念/想关心时用话语表达，比如“好想你”“要是现在能抱抱你就好了”“隔这么远好烦”。一句能说完就一句，确实像连续补话时再分成几条，别写成一大段或作文腔。');
  } else {
...
function renderPhoneIg(char, screen) {
  ...
  h += navItem('home', '首页', homeSvg) + navItem('dm', '私信', dmSvg) + navItem('me', '我的', meSvg);
  h += '</div>';
  h += '</div>';
  screen.innerHTML = h;
...
function renderPhoneIgHome(char, panel) {
  panel.innerHTML = '<div class="profile-panel-content"><div class="phone-empty">（空）</div></div>';
}
...
function renderPhoneAlbum(char, screen) {
  ...
  var photos = (char.phone && char.phone.photos) || [];
  if (!photos.length) { screen.innerHTML += '<div class="phone-empty">（相册里还没有照片）</div>'; return; }
  var h = '<div class="phone-album-grid">';
  photos.forEach(function (p) {
    var src = typeof p === 'string' ? p : (p.image || p.src || '');
    var cap = typeof p === 'string' ? '' : (p.caption || '');
    var inner = src ? '<img src="' + src + '" style="width:100%;height:100%;object-fit:cover">' : '🖼️';
    h += '<div class="phone-album-cell">' + inner + (cap ? '<div class="phone-album-cap">' + escapeHTML(cap) + '</div>' : '') + '</div>';
  });
  h += '</div>';
  screen.innerHTML += h;
}
